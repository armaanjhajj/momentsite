...
CREATE TYPE "public"."identity_delta_type" AS ENUM (
    'artifact_added',
    'artifact_removed',
    'status_changed',
    'bio_changed',
...
ALTER TYPE "public"."identity_delta_type" OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."archive_thought"("p_thought_id" "uuid", "p_user_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    UPDATE posts
    SET is_archived = TRUE
    WHERE id = p_thought_id AND user_id = p_user_id;
...
ALTER FUNCTION "public"."archive_thought"("p_thought_id" "uuid", "p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."award_points_busyness_report"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    UPDATE public.profiles SET moments_score = COALESCE(moments_score, 0) + 2 WHERE id = NEW.user_id;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."award_points_busyness_report"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."award_points_collectible_collected"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_points INTEGER;
BEGIN
  ...
  SET moments_score = COALESCE(moments_score, 0) + COALESCE(v_points, 0)
...
ALTER FUNCTION "public"."award_points_collectible_collected"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."award_points_comment_created"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    UPDATE public.profiles SET moments_score = COALESCE(moments_score, 0) + 3 WHERE id = NEW.user_id;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."award_points_comment_created"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."award_points_friendship_accepted"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    IF NEW.status = 'accepted' AND (OLD.status IS NULL OR OLD.status != 'accepted') THEN
        UPDATE public.profiles SET moments_score = COALESCE(moments_score, 0) + 5
...
ALTER FUNCTION "public"."award_points_friendship_accepted"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."award_points_moment_created"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    UPDATE public.profiles SET moments_score = COALESCE(moments_score, 0) + 10 WHERE id = NEW.user_id;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."award_points_moment_created"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."award_points_moment_liked"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    UPDATE public.profiles SET moments_score = COALESCE(moments_score, 0) + 1
    WHERE id IN (SELECT user_id FROM public.moments WHERE id = NEW.moment_id);
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."award_points_moment_liked"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."award_points_moment_viewed"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    UPDATE public.profiles SET moments_score = COALESCE(moments_score, 0) + 1 WHERE id = NEW.viewer_id;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."award_points_moment_viewed"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."award_points_spot_tip"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    UPDATE public.profiles SET moments_score = COALESCE(moments_score, 0) + 3 WHERE id = NEW.user_id;
...
ALTER FUNCTION "public"."award_points_spot_tip"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."can_message"("p_from_user" "uuid", "p_to_user" "uuid") RETURNS TABLE("allowed" boolean, "reason" "text", "conversation_exists" boolean, "is_friends" boolean)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    ordered_a UUID;
...
BEGIN
    -- Order users consistently for conversation check
    IF p_from_user < p_to_user THEN
        ordered_a := p_from_user;
        ordered_b := p_to_user;
    ELSE
        ordered_a := p_to_user;
        ordered_b := p_from_user;
    END IF;
    ...
    -- Check if there's an existing conversation with messages from both users
    IF conv_exists THEN
        ...
        IF EXISTS (
            ...
            RETURN QUERY SELECT TRUE, 'conversation_active'::TEXT, TRUE, FALSE;
            RETURN;
        END IF;
    END IF;

    -- Check friendship status
    SELECT get_friendship_status(p_from_user, p_to_user) INTO friendship;

    IF friendship = 'accepted' THEN
        RETURN QUERY SELECT TRUE, 'friends'::TEXT, conv_exists, TRUE;
        RETURN;
    END IF;

    -- Check if there's a pending message request (first message sent, waiting for response)
    IF conv_exists THEN
        -- Check if from_user has sent a message but to_user hasn't replied
        IF EXISTS (
            SELECT 1 FROM conversations c
            JOIN messages m ON m.conversation_id = c.id
            WHERE c.user_a = ordered_a AND c.user_b = ordered_b
            AND m.sender_id = p_from_user
            AND NOT EXISTS (
                SELECT 1 FROM messages m2
                WHERE m2.conversation_id = c.id
                AND m2.sender_id = p_to_user
            )
        ) THEN
            RETURN QUERY SELECT FALSE, 'request_pending'::TEXT, TRUE, FALSE;
            RETURN;
        END IF;
    END IF;

    -- Can send one message as a request
    RETURN QUERY SELECT TRUE, 'can_request'::TEXT, FALSE, FALSE;
END;
$$;


ALTER FUNCTION "public"."can_message"("p_from_user" "uuid", "p_to_user" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."check_in_to_spot"("p_spot_id" "uuid", "p_user_id" "uuid", "p_busyness_level" integer, "p_note" "text" DEFAULT NULL::"text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    INSERT INTO public.spot_busyness_reports (spot_id, user_id, busyness_level, note)
    VALUES (p_spot_id, p_user_id, p_busyness_level, p_note);
END;
$$;


ALTER FUNCTION "public"."check_in_to_spot"("p_spot_id" "uuid", "p_user_id" "uuid", "p_busyness_level" integer, "p_note" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."compute_user_similarity"() RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    TRUNCATE TABLE public.user_user_edges;
    INSERT INTO public.user_user_edges (user_a, user_b, weight, updated_at)
    SELECT
        e1.user_id AS user_a, e2.user_id AS user_b,
        SUM(
            (e1.weight * exp(-0.01 * extract(epoch FROM (now() - e1.created_at))/86400.0)) *
            (e2.weight * exp(-0.01 * extract(epoch FROM (now() - e2.created_at))/86400.0))
        ) / (
            SQRT(SUM(POWER(e1.weight * exp(-0.01 * extract(epoch FROM (now() - e1.created_at))/86400.0), 2))) *
            SQRT(SUM(POWER(e2.weight * exp(-0.01 * extract(epoch FROM (now() - e2.created_at))/86400.0), 2)))
        ) AS cosine_sim,
        NOW() AS updated_at
    FROM public.user_moment_edges e1
    JOIN public.user_moment_edges e2 ON e1.moment_id = e2.moment_id AND e1.user_id != e2.user_id
    GROUP BY e1.user_id, e2.user_id
    HAVING
        SUM((e1.weight * exp(-0.01 * extract(epoch FROM (now() - e1.created_at))/86400.0)) * (e2.weight * exp(-0.01 * extract(epoch FROM (now() - e2.created_at))/86400.0))) /
        (SQRT(SUM(POWER(e1.weight * exp(-0.01 * extract(epoch FROM (now() - e1.created_at))/86400.0), 2))) * SQRT(SUM(POWER(e2.weight * exp(-0.01 * extract(epoch FROM (now() - e2.created_at))/86400.0), 2)))) > 0.3;
END;
$$;


ALTER FUNCTION "public"."compute_user_similarity"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."create_default_archive_collection"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  INSERT INTO collections (user_id, title, description, is_archive, is_public)
  VALUES (NEW.id, 'Archive', 'Your archived moments', true, false)
...
ALTER FUNCTION "public"."create_default_archive_collection"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."expire_sotds"() RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    UPDATE profiles SET sotd = NULL, sotd_set_at = NULL
    WHERE sotd IS NOT NULL AND sotd_set_at < CURRENT_DATE;
END;
$$;


ALTER FUNCTION "public"."expire_sotds"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."find_mutual_contacts"("current_user_id" "uuid") RETURNS TABLE("user_id" "uuid", "name" "text", "handle" "text", "photo_url" "text")
    LANGUAGE "sql" STABLE SECURITY DEFINER
    AS $$
    select u.id as user_id, u.name, u.handle, u.photo_url
    from public.users u
    where u.id != current_user_id
      -- They have my phone in their contacts
      and exists (
          select 1 from public.user_contacts uc
          where uc.user_id = u.id
            and uc.phone = (select phone from public.users where id = current_user_id)
      )
      -- I have their phone in my contacts
      and exists (
          select 1 from public.user_contacts uc
          where uc.user_id = current_user_id
            and uc.phone = u.phone
      );
$$;


ALTER FUNCTION "public"."find_mutual_contacts"("current_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_all_collections"() RETURNS TABLE("collection_id" "uuid", "title" "text", "description" "text", "image_url" "text", "owner_id" "uuid", "owner_first_name" "text", "owner_last_name" "text", "owner_handle" "text", "owner_avatar_url" "text", "is_archive" boolean, "is_public" boolean, "moment_count" integer, "contributor_ids" "uuid"[], "created_at" timestamp with time zone, "first_moment_image" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    c.moment_count,
    -- COALESCE(ca.contributor_ids, ARRAY[]::uuid[]),
    ARRAY[]::uuid[] as contributor_ids,
    c.created_at,
    (
      SELECT m.image_url
      ...
      ORDER BY cm.display_order ASC, cm.added_at DESC
      LIMIT 1
    ) as first_moment_image
  FROM collections c
  JOIN profiles p ON c.user_id = p.id
  -- LEFT JOIN collection_authors ca ON c.id = ca.collection_id
...
ALTER FUNCTION "public"."get_all_collections"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_campus_spots_near"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision DEFAULT 5.0, "p_school_id" "text" DEFAULT NULL::"text", "p_accessible_only" boolean DEFAULT false) RETURNS TABLE("id" "uuid", "name" "text", "description" "text", "emoji" "text", "spot_type" "text", "location_lat" double precision, "location_lng" double precision, "location_name" "text", "school_id" "text", "hours" "jsonb", "has_wifi" boolean, "has_outlets" boolean, "has_food" boolean, "noise_level" "text", "is_accessible" boolean, "accessibility_notes" "text", "distance_miles" double precision, "avg_busyness" numeric, "recent_reports_count" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
    SELECT s.id, s.name, s.description, s.emoji, s.spot_type,
        s.location_lat, s.location_lng, s.location_name, s.school_id, s.hours,
        s.has_wifi, s.has_outlets, s.has_food, s.noise_level, s.is_accessible,
        s.accessibility_notes,
        (3958.8 * acos(LEAST(1.0, cos(radians(p_lat)) * cos(radians(s.location_lat)) *
            cos(radians(s.location_lng) - radians(p_lng)) + sin(radians(p_lat)) * sin(radians(s.location_lat))
        ))) AS distance_miles,
        (SELECT ROUND(AVG(br.busyness_level), 1) FROM public.spot_busyness_reports br
         WHERE br.spot_id = s.id AND br.reported_at > NOW() - INTERVAL '2 hours') AS avg_busyness,
        (SELECT COUNT(*) FROM public.spot_busyness_reports br
         WHERE br.spot_id = s.id AND br.reported_at > NOW() - INTERVAL '2 hours') AS recent_reports_count
    FROM public.campus_spots s
    WHERE s.is_active = TRUE
    AND (p_school_id IS NULL OR s.school_id IS NULL OR s.school_id = p_school_id)
    AND (p_accessible_only = FALSE OR s.is_accessible = TRUE)
    AND (3958.8 * acos(LEAST(1.0, cos(radians(p_lat)) * cos(radians(s.location_lat)) *
        cos(radians(s.location_lng) - radians(p_lng)) + sin(radians(p_lat)) * sin(radians(s.location_lat))
    ))) <= p_radius_miles
...
ALTER FUNCTION "public"."get_campus_spots_near"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_accessible_only" boolean) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_challenge_completions"("p_challenge_id" "uuid", "p_limit" integer DEFAULT 20) RETURNS TABLE("completion_id" "uuid", "user_id" "uuid", "user_first_name" "text", "user_handle" "text", "user_avatar_url" "text", "moment_id" "uuid", "moment_image_url" "text", "moment_video_thumbnail_url" "text", "moment_text" "text", "completed_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    cc.moment_id,
    m.image_url AS moment_image_url,
    m.video_thumbnail_url AS moment_video_thumbnail_url,
    m.text AS moment_text,
    cc.completed_at
  FROM public.challenge_completions cc
  JOIN public.profiles p ON p.id = cc.user_id
  LEFT JOIN public.moments m ON m.id = cc.moment_id
...
ALTER FUNCTION "public"."get_challenge_completions"("p_challenge_id" "uuid", "p_limit" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_challenges_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision DEFAULT 5.0, "p_school_id" "text" DEFAULT NULL::"text", "p_user_id" "uuid" DEFAULT NULL::"uuid") RETURNS TABLE("id" "uuid", "title" "text", "description" "text", "emoji" "text", "challenge_type" "text", "location_type" "text", "location_lat" double precision, "location_lng" double precision, "radius_ft" integer, "location_name" "text", "school_id" "text", "points_reward" integer, "expires_at" timestamp with time zone, "completion_count" integer, "created_at" timestamp with time zone, "distance_miles" double precision, "user_completed" boolean)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    c.created_at,
    CASE
      WHEN c.location_type = 'locked' AND c.location_lat IS NOT NULL THEN
        (3958.8 * acos(
          LEAST(1.0, cos(radians(p_lat)) * cos(radians(c.location_lat)) *
          cos(radians(c.location_lng) - radians(p_lng)) +
          sin(radians(p_lat)) * sin(radians(c.location_lat)))
        ))
      ELSE NULL
    END AS distance_miles,
    EXISTS(
      SELECT 1 FROM public.challenge_completions cc
      WHERE cc.challenge_id = c.id AND cc.user_id = p_user_id
    ) AS user_completed
  FROM public.challenges c
  WHERE
    c.is_active = TRUE
    AND (c.expires_at IS NULL OR c.expires_at > NOW())
    AND (p_school_id IS NULL OR c.school_id IS NULL OR c.school_id = p_school_id)
    AND (
      c.location_type = 'open'
      OR (
        c.location_type = 'locked'
  ...
  ORDER BY distance_miles ASC NULLS LAST, c.created_at DESC;
END;
$$;


ALTER FUNCTION "public"."get_challenges_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_collectibles_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision DEFAULT 5.0, "p_school_id" "text" DEFAULT NULL::"text", "p_user_id" "uuid" DEFAULT NULL::"uuid") RETURNS TABLE("id" "uuid", "name" "text", "description" "text", "emoji" "text", "rarity" "text", "location_lat" double precision, "location_lng" double precision, "radius_ft" integer, "location_name" "text", "school_id" "text", "points_reward" integer, "total_collected" integer, "distance_miles" double precision, "user_collected" boolean, "collected_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    EXISTS(
      ...
      WHERE uc.collectible_id = c.id AND uc.user_id = p_user_id
    ) AS user_collected,
    (
      SELECT uc.collected_at FROM public.user_collectibles uc
      WHERE uc.collectible_id = c.id AND uc.user_id = p_user_id
    ) AS collected_at
...
ALTER FUNCTION "public"."get_collectibles_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_friend_moments"("requesting_user_id" "uuid") RETURNS TABLE("moment_id" "uuid", "user_id" "uuid", "text" "text", "image_url" "text", "video_url" "text", "video_thumbnail_url" "text", "location_name" "text", "expires_at" timestamp with time zone, "created_at" timestamp with time zone, "author_first_name" "text", "author_last_name" "text", "author_handle" "text", "author_avatar_url" "text", "has_viewed" boolean, "has_liked" boolean, "like_count" integer, "comment_count" integer, "share_count" integer, "views_count" integer)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        m.id AS moment_id, m.user_id, m.text, m.image_url, m.video_url,
        m.video_thumbnail_url, m.location_name, m.expires_at, m.created_at,
        p.first_name AS author_first_name, NULL::TEXT AS author_last_name,
        p.handle AS author_handle, p.avatar_url AS author_avatar_url,
        EXISTS(SELECT 1 FROM moment_views mv WHERE mv.moment_id = m.id AND mv.viewer_id = requesting_user_id) AS has_viewed,
        EXISTS(SELECT 1 FROM moment_likes ml WHERE ml.moment_id = m.id AND ml.user_id = requesting_user_id) AS has_liked,
        (SELECT COALESCE(SUM(ml.amount), 0)::INT FROM moment_likes ml WHERE ml.moment_id = m.id) AS like_count,
        COALESCE(m.comment_count, 0)::INT AS comment_count,
        (SELECT COUNT(*)::INT FROM messages msg WHERE msg.moment_id = m.id) AS share_count,
        COALESCE(m.views_count, 0)::INT AS views_count
    FROM moments m
    JOIN profiles p ON p.id = m.user_id
    WHERE m.expires_at > NOW()
    AND NOT m.is_archived
    AND m.visibility != 'geocached'
    AND m.visibility != 'vault'
    AND (
        m.user_id = requesting_user_id
        OR EXISTS(
            SELECT 1 FROM friendships f WHERE f.status = 'accepted'
            AND ((f.user_a = requesting_user_id AND f.user_b = m.user_id) OR (f.user_b = requesting_user_id AND f.user_a = m.user_id))
        )
    )
...
ALTER FUNCTION "public"."get_friend_moments"("requesting_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_friend_of_friend_candidates"("p_user_id" "uuid", "p_limit" integer DEFAULT 50) RETURNS TABLE("candidate_id" "uuid", "mutual_count" bigint)
...
BEGIN
    ...
    WITH my_friends AS (
        ...
        WHERE f.status = 'accepted' AND (f.user_a = p_user_id OR f.user_b = p_user_id)
    ),
    fof AS (
        SELECT
            CASE WHEN f2.user_a = mf.friend_id THEN f2.user_b ELSE f2.user_a END AS candidate,
            mf.friend_id AS via_friend
        FROM my_friends mf
        JOIN public.friendships f2 ON f2.status = 'accepted' AND (f2.user_a = mf.friend_id OR f2.user_b = mf.friend_id)
        WHERE CASE WHEN f2.user_a = mf.friend_id THEN f2.user_b ELSE f2.user_a END != p_user_id
    )
    SELECT fof.candidate AS candidate_id, COUNT(DISTINCT fof.via_friend) AS mutual_count
    FROM fof WHERE fof.candidate NOT IN (SELECT friend_id FROM my_friends)
    GROUP BY fof.candidate ORDER BY mutual_count DESC LIMIT p_limit;
END;
$$;


ALTER FUNCTION "public"."get_friend_of_friend_candidates"("p_user_id" "uuid", "p_limit" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_friendship_statuses_bulk"("p_user_id" "uuid", "p_target_ids" "uuid"[]) RETURNS TABLE("target_id" "uuid", "status" "text", "friendship_id" "uuid", "initiated_by" "uuid")
    LANGUAGE "plpgsql" STABLE
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        CASE WHEN f.user_a = p_user_id THEN f.user_b ELSE f.user_a END AS target_id,
        f.status::TEXT AS status, f.id AS friendship_id, f.initiated_by
    FROM public.friendships f
    WHERE (f.user_a = p_user_id AND f.user_b = ANY(p_target_ids))
       OR (f.user_b = p_user_id AND f.user_a = ANY(p_target_ids));
END;
$$;


ALTER FUNCTION "public"."get_friendship_statuses_bulk"("p_user_id" "uuid", "p_target_ids" "uuid"[]) OWNER TO "postgres";
...
BEGIN
    IF user1_id < user2_id THEN
        ordered_user_a := user1_id;
        ordered_user_b := user2_id;
    ELSE
        ordered_user_a := user2_id;
        ordered_user_b := user1_id;
    END IF;
    SELECT id INTO conversation_id FROM conversations
    WHERE user_a = ordered_user_a AND user_b = ordered_user_b;
    IF conversation_id IS NULL THEN
        INSERT INTO conversations (user_a, user_b) VALUES (ordered_user_a, ordered_user_b)
...
CREATE OR REPLACE FUNCTION "public"."get_oracle_recommendations"("p_limit" integer DEFAULT 10) RETURNS TABLE("id" "uuid", "first_name" "text", "last_name" "text", "username" "text", "avatar_url" "text", "similarity" double precision, "shared_context" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
    SELECT u.id, u.first_name, NULL::TEXT AS last_name, u.handle AS username, u.avatar_url,
        ue.weight AS similarity,
        (SELECT CASE
            WHEN m.location_name IS NOT NULL THEN 'You both visited ' || m.location_name
            WHEN m.text IS NOT NULL THEN 'You both spent time on: "' || SUBSTRING(m.text FROM 1 FOR 35) || '..."'
            ELSE 'You have similar tastes in moments'
        END
        FROM public.moments m
        JOIN public.user_moment_edges e1 ON e1.moment_id = m.id AND e1.user_id = auth.uid()
        JOIN public.user_moment_edges e2 ON e2.moment_id = m.id AND e2.user_id = u.id
        ORDER BY e1.weight DESC, e2.weight DESC LIMIT 1) AS shared_context
    FROM public.profiles u
    JOIN public.user_user_edges ue ON ue.user_b = u.id AND ue.user_a = auth.uid()
    WHERE ue.weight > 0.4 AND u.onboarding_completed = TRUE
    ORDER BY ue.weight DESC LIMIT p_limit;
END;
$$;


ALTER FUNCTION "public"."get_oracle_recommendations"("p_limit" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_popular_spots_nearby"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision DEFAULT 5, "p_limit" integer DEFAULT 20) RETURNS TABLE("name" "text", "lat" double precision, "lng" double precision, "save_count" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
  SELECT ss.name, ss.lat, ss.lng, COUNT(*)::BIGINT as save_count
  FROM public.saved_spots ss
  WHERE (ss.lat - p_lat)^2 + (ss.lng - p_lng)^2 < (p_radius_miles / 69.0)^2
  GROUP BY ss.name, ss.lat, ss.lng
...
ALTER FUNCTION "public"."get_popular_spots_nearby"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_limit" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_profile_compliments"("target_profile_id" "uuid") RETURNS TABLE("category" "text", "count" bigint, "endorsed" boolean)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    AS $$
    SELECT
        pc.category,
        COUNT(*)::bigint AS count,
        BOOL_OR(pc.author_id = auth.uid()) AS endorsed
    FROM public.profile_compliments pc
...
ALTER FUNCTION "public"."get_profile_compliments"("target_profile_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_social_graph"("p_user_id" "uuid", "p_depth" integer DEFAULT 2) RETURNS TABLE("graph_user_id" "uuid", "first_name" "text", "last_name" "text", "avatar_url" "text", "depth" integer, "connection_through" "uuid")
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    -- First level: direct friends
    RETURN QUERY
    WITH friends AS (
        SELECT 
    ...
    friends_of_friends AS (
        SELECT 
            CASE WHEN f2.user_a = fr.friend_id THEN f2.user_b ELSE f2.user_a END as fof_id,
            fr.friend_id as through_friend
        FROM friends fr
        JOIN friendships f2 ON (f2.user_a = fr.friend_id OR f2.user_b = fr.friend_id)
        WHERE f2.status = 'accepted'
        AND CASE WHEN f2.user_a = fr.friend_id THEN f2.user_b ELSE f2.user_a END != p_user_id
        AND CASE WHEN f2.user_a = fr.friend_id THEN f2.user_b ELSE f2.user_a END NOT IN (SELECT friend_id FROM friends)
    )
    -- Direct friends (depth 1)
    SELECT 
        p.id as graph_user_id,
        p.first_name,
        p.last_name,
        p.avatar_url,
        1 as depth,
        NULL::UUID as connection_through
    FROM profiles p
    ...
    SELECT 
        ...
        p.avatar_url,
        2 as depth,
        fof.through_friend as connection_through
    FROM profiles p
    JOIN friends_of_friends fof ON p.id = fof.fof_id;
END;
$$;


ALTER FUNCTION "public"."get_social_graph"("p_user_id" "uuid", "p_depth" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_spot_tips"("p_spot_id" "uuid", "p_limit" integer DEFAULT 20) RETURNS TABLE("id" "uuid", "tip_text" "text", "upvotes" integer, "user_first_name" "text", "user_handle" "text", "user_avatar_url" "text", "created_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
    SELECT t.id, t.tip_text, t.upvotes, p.first_name, p.handle, p.avatar_url, t.created_at
    FROM public.spot_tips t JOIN public.profiles p ON p.id = t.user_id
    WHERE t.spot_id = p_spot_id ORDER BY t.upvotes DESC, t.created_at DESC LIMIT p_limit;
END;
$$;


ALTER FUNCTION "public"."get_spot_tips"("p_spot_id" "uuid", "p_limit" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_suggested_profiles"("p_user_id" "uuid", "p_limit" integer DEFAULT 10) RETURNS TABLE("suggested_user_id" "uuid", "first_name" "text", "last_name" "text", "handle" "text", "headline" "text", "avatar_url" "text", "top_artists" "text"[], "favorite_activities" "text"[], "school" "text", "match_reason" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
    ...
    SELECT 
        ...
        p.school,
        CASE 
            WHEN p.school = up.school AND up.school IS NOT NULL THEN 'same_school'
            WHEN p.top_artists && up.top_artists THEN 'shared_artists'
            WHEN p.favorite_activities && up.favorite_activities THEN 'shared_activities'
            ELSE 'nearby'
        END as match_reason
    FROM profiles p
    ...
    AND NOT EXISTS (
        ...
        WHERE (f.user_a = p_user_id AND f.user_b = p.id)
        OR (f.user_a = p.id AND f.user_b = p_user_id)
    )
    AND NOT EXISTS (
        SELECT 1 FROM blocked_users bu 
        WHERE (bu.user_id = p_user_id AND bu.blocked_user_id = p.id)
        OR (bu.user_id = p.id AND bu.blocked_user_id = p_user_id)
    )
    ORDER BY 
        CASE 
            WHEN p.school = up.school AND up.school IS NOT NULL THEN 1
            WHEN p.top_artists && up.top_artists THEN 2
            WHEN p.favorite_activities && up.favorite_activities THEN 3
            ELSE 4
        END,
        RANDOM()
    LIMIT p_limit;
...
ALTER FUNCTION "public"."get_suggested_profiles"("p_user_id" "uuid", "p_limit" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_top_rooms"("limit_count" integer DEFAULT 10) RETURNS TABLE("room_id" "uuid", "name" "text", "description" "text", "image_url" "text", "member_count" bigint, "active_count" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        r.id AS room_id, r.name, r.description, r.image_url,
        (SELECT COUNT(*) FROM room_members WHERE room_id = r.id) AS member_count,
        (SELECT COUNT(*) FROM user_presence WHERE current_room_id = r.id AND status = 'online') AS active_count
    FROM rooms r WHERE r.visibility = 'public' AND NOT r.is_archived
    ORDER BY active_count DESC, member_count DESC LIMIT limit_count;
END;
$$;


ALTER FUNCTION "public"."get_top_rooms"("limit_count" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_user_collectibles"("p_user_id" "uuid") RETURNS TABLE("id" "uuid", "name" "text", "description" "text", "emoji" "text", "rarity" "text", "location_name" "text", "points_reward" integer, "collected_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
...
ALTER FUNCTION "public"."get_user_collectibles"("p_user_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."increment_invites"("uid" "uuid") RETURNS "void"
...
ALTER FUNCTION "public"."increment_invites"("uid" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."increment_moment_like"("p_moment_id" "uuid", "p_user_id" "uuid", "p_amount" integer) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    INSERT INTO public.moment_likes (moment_id, user_id, amount)
    VALUES (p_moment_id, p_user_id, p_amount)
    ON CONFLICT (moment_id, user_id)
    DO UPDATE SET amount = LEAST(public.moment_likes.amount + p_amount, 999);
END;
$$;


ALTER FUNCTION "public"."increment_moment_like"("p_moment_id" "uuid", "p_user_id" "uuid", "p_amount" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."increment_moment_views"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    UPDATE moments SET views_count = views_count + 1 WHERE id = NEW.moment_id;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."increment_moment_views"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."log_interaction"("p_moment_id" "uuid", "p_action" "text", "p_duration_ms" integer DEFAULT NULL::integer) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    v_user_id UUID; v_base_weight FLOAT8 := 0; v_weight FLOAT8;
BEGIN
    v_user_id := auth.uid();
    IF v_user_id IS NULL THEN RETURN; END IF;
    CASE p_action
        WHEN 'view' THEN v_base_weight := 0.3;
        WHEN 'like' THEN v_base_weight := 1.0;
        WHEN 'comment' THEN v_base_weight := 1.2;
        WHEN 'share' THEN v_base_weight := 1.5;
        WHEN 'skip' THEN v_base_weight := -0.3;
        ELSE v_base_weight := 0;
    END CASE;
    v_weight := v_base_weight;
    IF p_duration_ms IS NOT NULL AND p_duration_ms > 0 THEN
        v_weight := v_weight * LEAST(p_duration_ms::FLOAT8 / 30000.0, 2.0);
    END IF;
    INSERT INTO public.user_moment_edges (user_id, moment_id, action, weight, duration_ms)
    VALUES (v_user_id, p_moment_id, p_action, v_weight, p_duration_ms);
END;
$$;


ALTER FUNCTION "public"."log_interaction"("p_moment_id" "uuid", "p_action" "text", "p_duration_ms" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."record_thought_view"("p_thought_id" "uuid", "p_viewer_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    thought_owner UUID;
BEGIN
    -- Get the thought owner
    SELECT user_id INTO thought_owner FROM posts WHERE id = p_thought_id;
    
    -- Don't count views from the owner
    IF thought_owner = p_viewer_id THEN
        RETURN FALSE;
    END IF;

    -- Try to insert the view (will fail silently on duplicate)
    INSERT INTO thought_views (thought_id, viewer_id)
    VALUES (p_thought_id, p_viewer_id)
    ON CONFLICT (thought_id, viewer_id) DO NOTHING;

    -- Update the view count
    UPDATE posts 
    SET views_count = (SELECT COUNT(*) FROM thought_views WHERE thought_id = p_thought_id)
...
ALTER FUNCTION "public"."record_thought_view"("p_thought_id" "uuid", "p_viewer_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."search_connections"("query_embedding" "public"."vector", "current_user_id" "uuid", "match_count" integer DEFAULT 20) RETURNS TABLE("id" "uuid", "similarity" double precision)
    LANGUAGE "sql" STABLE
    AS $$
    select u.id,
    ...
    where  u.embedding is not null
      ...
      and  exists (
               ...
               where  (c.user_a = current_user_id and c.user_b = u.id)
                  or  (c.user_b = current_user_id and c.user_a = u.id)
           )
...
ALTER FUNCTION "public"."search_connections"("query_embedding" "public"."vector", "current_user_id" "uuid", "match_count" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."search_moments_v2"("p_query_embedding" "public"."vector", "p_user_id" "uuid" DEFAULT NULL::"uuid", "p_match_threshold" double precision DEFAULT 0.3, "p_match_count" integer DEFAULT 50, "p_date_from" timestamp with time zone DEFAULT NULL::timestamp with time zone, "p_date_to" timestamp with time zone DEFAULT NULL::timestamp with time zone, "p_lat" double precision DEFAULT NULL::double precision, "p_lng" double precision DEFAULT NULL::double precision, "p_radius_miles" double precision DEFAULT NULL::double precision, "p_school" "text" DEFAULT NULL::"text") RETURNS TABLE("out_id" "uuid", "out_title" "text", "out_caption" "text", "out_created_at" timestamp with time zone, "out_location_name" "text", "out_location_lat" double precision, "out_location_lng" double precision, "out_image_url" "text", "out_video_thumbnail_url" "text", "out_user_id" "uuid", "out_moment_type" "text", "out_media_title" "text", "out_media_artist" "text", "out_author_handle" "text", "out_author_first_name" "text", "out_author_avatar_url" "text", "out_school" "text", "out_similarity" double precision)
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public'
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        m.id, m.title, m.caption, m.created_at, m.location_name, m.location_lat, m.location_lng,
        m.image_url, m.video_thumbnail_url, m.user_id, m.moment_type::TEXT,
        m.media_title, m.media_artist, p.handle, p.first_name, p.avatar_url,
        p.school, (1 - (m.moment_embedding <=> p_query_embedding))::FLOAT
    FROM moments m JOIN profiles p ON p.id = m.user_id
    WHERE m.is_archived = FALSE AND m.moment_embedding IS NOT NULL AND m.expires_at > NOW()
    AND (1 - (m.moment_embedding <=> p_query_embedding)) >= p_match_threshold
    ...
    AND (p_radius_miles IS NULL OR p_lat IS NULL OR p_lng IS NULL OR (
        m.location_lat IS NOT NULL AND m.location_lng IS NOT NULL
        AND (3959.0 * acos(LEAST(1.0, GREATEST(-1.0,
            cos(radians(p_lat)) * cos(radians(m.location_lat)) * cos(radians(m.location_lng) - radians(p_lng))
            + sin(radians(p_lat)) * sin(radians(m.location_lat))
        )))) <= p_radius_miles
    ))
    ORDER BY m.moment_embedding <=> p_query_embedding LIMIT p_match_count;
END;
$$;


ALTER FUNCTION "public"."search_moments_v2"("p_query_embedding" "public"."vector", "p_user_id" "uuid", "p_match_threshold" double precision, "p_match_count" integer, "p_date_from" timestamp with time zone, "p_date_to" timestamp with time zone, "p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."search_oracle_profiles"("p_tags" "text"[], "p_limit" integer DEFAULT 10) RETURNS TABLE("out_id" "uuid", "out_first_name" "text", "out_last_name" "text", "out_handle" "text", "out_avatar_url" "text", "out_school" "text", "out_headline" "text", "out_oracle_tags" "text"[], "out_match_score" double precision)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
    SELECT u.id, u.first_name, NULL::TEXT, u.handle, u.avatar_url, u.school, u.headline, u.oracle_tags,
        (
            COALESCE((SELECT COUNT(*) FROM unnest(p_tags) t WHERE t = ANY(u.oracle_tags)), 0)::FLOAT8
            + COALESCE((SELECT COUNT(*) FROM unnest(p_tags) t WHERE EXISTS (
                SELECT 1 FROM unnest(u.oracle_tags) ot WHERE ot ILIKE '%' || t || '%' OR t ILIKE '%' || ot || '%'
            )), 0)::FLOAT8 * 0.5
            + COALESCE((SELECT ue.weight FROM public.user_user_edges ue WHERE (ue.user_a = auth.uid() AND ue.user_b = u.id) OR (ue.user_a = u.id AND ue.user_b = auth.uid()) LIMIT 1), 0.0)
        ) AS match_score
    FROM public.profiles u
    WHERE u.id != auth.uid() AND u.oracle_tags IS NOT NULL AND array_length(u.oracle_tags, 1) > 0
    ORDER BY match_score DESC LIMIT p_limit;
END;
$$;


ALTER FUNCTION "public"."search_oracle_profiles"("p_tags" "text"[], "p_limit" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."search_people_semantic"("p_requesting_user_id" "uuid", "p_query" "text", "p_user_lat" double precision, "p_user_lon" double precision, "p_radius_miles" double precision DEFAULT 25) RETURNS TABLE("user_id" "uuid", "first_name" "text", "last_name" "text", "handle" "text", "headline" "text", "avatar_url" "text", "distance_miles" double precision, "favorite_activities" "text"[], "top_artists" "text"[], "school" "text", "match_score" double precision)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN QUERY
    SELECT 
        ...
        (
            CASE WHEN p.headline ILIKE '%' || p_query || '%' THEN 3 ELSE 0 END +
            CASE WHEN array_to_string(p.favorite_activities, ',') ILIKE '%' || p_query || '%' THEN 2 ELSE 0 END +
            CASE WHEN array_to_string(p.top_artists, ',') ILIKE '%' || p_query || '%' THEN 2 ELSE 0 END +
            CASE WHEN p.school ILIKE '%' || p_query || '%' THEN 2 ELSE 0 END +
            CASE WHEN p.first_name ILIKE '%' || p_query || '%' THEN 1 ELSE 0 END
    ...
    AND NOT EXISTS (
        SELECT 1 FROM blocked_users bu 
        WHERE (bu.user_id = p_requesting_user_id AND bu.blocked_user_id = p.id)
        OR (bu.user_id = p.id AND bu.blocked_user_id = p_requesting_user_id)
    )
    ORDER BY match_score DESC, distance_miles ASC
    LIMIT 3; -- Only return 1-3 results as per requirements
END;
$$;


ALTER FUNCTION "public"."search_people_semantic"("p_requesting_user_id" "uuid", "p_query" "text", "p_user_lat" double precision, "p_user_lon" double precision, "p_radius_miles" double precision) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."search_profiles_v2"("search_query" "text" DEFAULT NULL::"text", "match_count" integer DEFAULT 20, "exclude_user_id" "uuid" DEFAULT NULL::"uuid", "filter_gender" "text" DEFAULT NULL::"text", "filter_school" "text" DEFAULT NULL::"text", "filter_ethnicity" "text"[] DEFAULT NULL::"text"[]) RETURNS TABLE("out_id" "uuid", "out_first_name" "text", "out_last_name" "text", "out_handle" "text", "out_headline" "text", "out_avatar_url" "text", "out_school" "text", "out_gender" "text", "out_ethnicity" "text", "out_top_artists" "text"[], "out_favorite_activities" "text"[], "out_identity_tags" "text"[], "out_search_text" "text", "out_similarity" double precision, "out_birthday" "date", "out_graduation_year" integer)
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public'
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        p.id, p.first_name, NULL::TEXT, p.handle, p.headline, p.avatar_url, p.school,
        p.gender,
        (CASE WHEN p.ethnicity IS NOT NULL AND array_length(p.ethnicity, 1) > 0 THEN array_to_string(p.ethnicity, ', ') ELSE NULL END),
        COALESCE(p.top_artists, '{}'::TEXT[]),
        COALESCE(p.interests, '{}'::TEXT[]),
        '{}'::TEXT[],
        array_to_string(COALESCE(p.search_tags, '{}'::TEXT[]) || COALESCE(p.interests, '{}'::TEXT[]), ' '),
        0.7::FLOAT,
        p.birthday,
        p.graduation_year
    FROM profiles p
    ...
    AND (filter_ethnicity IS NULL OR p.ethnicity && filter_ethnicity)
    AND (search_query IS NULL OR search_query = ''
        ...
        OR array_to_string(COALESCE(p.search_tags, '{}'), ' ') ILIKE '%' || search_query || '%'
        OR array_to_string(COALESCE(p.interests, '{}'), ' ') ILIKE '%' || search_query || '%')
    ORDER BY
        CASE
            WHEN search_query IS NULL OR search_query = '' THEN 0
            WHEN p.first_name ILIKE '%' || search_query || '%' THEN 3
            WHEN p.handle ILIKE '%' || search_query || '%' THEN 2
            ELSE 1
        END DESC
    LIMIT match_count;
END;
...
ALTER FUNCTION "public"."search_profiles_v2"("search_query" "text", "match_count" integer, "exclude_user_id" "uuid", "filter_gender" "text", "filter_school" "text", "filter_ethnicity" "text"[]) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."update_moment_comment_count"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
...
ALTER FUNCTION "public"."update_moment_comment_count"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."upsert_chunk_visit"("p_user_id" "uuid", "p_chunk_id" "text", "p_source" "text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_body TEXT;
  ...
  v_step DOUBLE PRECISION := 0.5 / 69.0;  -- CHUNK_SIZE_MILES / MILES_PER_DEGREE_LAT
BEGIN
  -- Parse "chunk_{lat}_{lng}" — lat is always the first segment after the
  -- prefix; lng is everything after the first underscore (may be negative).
  v_body := replace(p_chunk_id, 'chunk_', '');
  v_sep  := position('_' IN v_body);
  v_lat  := CAST(substring(v_body FROM 1 FOR v_sep - 1) AS DOUBLE PRECISION);
  v_lng  := CAST(substring(v_body FROM v_sep + 1) AS DOUBLE PRECISION);

  -- Create the chunk row if it doesn't exist
  INSERT INTO public.location_chunks (chunk_id, lat_min, lat_max, lng_min, lng_max, total_weight)
  VALUES (
    p_chunk_id,
    v_lat,
    v_lat + v_step,
    v_lng,
    v_lng + v_step / cos(v_lat * pi() / 180),
    1
  )
  ON CONFLICT (chunk_id) DO UPDATE
    SET total_weight = public.location_chunks.total_weight + 1;

  -- Upsert user chunk history
  INSERT INTO public.user_chunk_history (user_id, chunk_id, source, weight, first_seen_at, last_seen_at, visit_count)
  VALUES (p_user_id, p_chunk_id, p_source, 1, now(), now(), 1)
  ON CONFLICT (user_id, chunk_id, source) DO UPDATE
    SET
      weight = public.user_chunk_history.weight + 1,
      visit_count = public.user_chunk_history.visit_count + 1,
      last_seen_at = now();
END;
$$;


ALTER FUNCTION "public"."upsert_chunk_visit"("p_user_id" "uuid", "p_chunk_id" "text", "p_source" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."upsert_user_location"("p_user_id" "uuid", "p_latitude" double precision, "p_longitude" double precision, "p_accuracy" double precision DEFAULT NULL::double precision) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  INSERT INTO public.user_locations (user_id, latitude, longitude, location, accuracy, updated_at)
  VALUES (
    p_user_id,
    p_latitude,
    p_longitude,
...
ALTER FUNCTION "public"."upsert_user_location"("p_user_id" "uuid", "p_latitude" double precision, "p_longitude" double precision, "p_accuracy" double precision) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."verify_admin_password"("pw" "text") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    RETURN EXISTS (
...
ALTER FUNCTION "public"."verify_admin_password"("pw" "text") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."activity" (
    ...
    CONSTRAINT "activity_event_type_check" CHECK (("event_type" = ANY (ARRAY['new_connection'::"text", 'card_viewed'::"text", 'went_open'::"text", 'went_closed'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."box_items" (
    ...
    CONSTRAINT "box_items_slot_check" CHECK ((("slot" >= 0) AND ("slot" <= 4))),
    CONSTRAINT "box_items_type_check" CHECK (("type" = ANY (ARRAY['song'::"text", 'movie'::"text", 'book'::"text", 'image'::"text", 'text'::"text"])))
);
...
ALTER TABLE "public"."card_views" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."connections" (
    ...
    "method" "text",
    "location_note" "text",
    "lat_zone" double precision,
    "lng_zone" double precision,
    "connected_at" timestamp without time zone DEFAULT "now"(),
    CONSTRAINT "connections_method_check" CHECK (("method" = ANY (ARRAY['namedrop'::"text", 'in_app'::"text", 'manual'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."notifications" (
    ...
    "related_user" "uuid",
    "created_at" timestamp without time zone DEFAULT "now"(),
    CONSTRAINT "notifications_type_check" CHECK (("type" = ANY (ARRAY['pulse'::"text", 'connection_request'::"text", 'new_connection'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."presence" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
    "lat_zone" double precision NOT NULL,
    "lng_zone" double precision NOT NULL,
    "venue_name" "text",
...
CREATE TABLE IF NOT EXISTS "public"."proximity_events" (
    ...
    "lat_zone" double precision NOT NULL,
    "lng_zone" double precision NOT NULL,
    "nearby_count" integer DEFAULT 0 NOT NULL,
    "notified_at" timestamp without time zone DEFAULT "now"()
);


ALTER TABLE "public"."proximity_events" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."taste_items" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "taste_items_category_check" CHECK (("category" = ANY (ARRAY['song'::"text", 'album'::"text", 'artist'::"text", 'movie'::"text", 'tvshow'::"text", 'book'::"text"])))
);


ALTER TABLE "public"."taste_items" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."users" (
    "id" "uuid" NOT NULL,
    "phone" "text",
    "handle" "text" DEFAULT ('user_'::"text" || "substr"(("gen_random_uuid"())::"text", 1, 8)) NOT NULL,
    "name" "text" DEFAULT 'New User'::"text" NOT NULL,
    "headline" "text",
    "photo_url" "text",
    "card_color" "text" DEFAULT '#000000'::"text",
    "is_open" boolean DEFAULT false,
    ...
    "impulse_score" integer DEFAULT 0 NOT NULL,
    "birthday" "date",
...
ALTER TABLE ONLY "public"."box_items"
    ADD CONSTRAINT "box_items_user_slot_unique" UNIQUE ("user_id", "slot");
...
ALTER TABLE ONLY "public"."connection_requests"
    ADD CONSTRAINT "connection_requests_sender_id_receiver_id_key" UNIQUE ("sender_id", "receiver_id");
...
ALTER TABLE ONLY "public"."proximity_events"
    ADD CONSTRAINT "proximity_events_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."taste_items"
    ADD CONSTRAINT "taste_items_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."user_contacts"
    ADD CONSTRAINT "user_contacts_user_phone_unique" UNIQUE ("user_id", "phone");
...
CREATE INDEX "box_items_user_id_idx" ON "public"."box_items" USING "btree" ("user_id");
...
CREATE UNIQUE INDEX "presence_user_id_idx" ON "public"."presence" USING "btree" ("user_id");



CREATE UNIQUE INDEX "presence_user_idx" ON "public"."presence" USING "btree" ("user_id");



CREATE INDEX "taste_items_user_id_idx" ON "public"."taste_items" USING "btree" ("user_id", "category", "sort_order");



CREATE INDEX "user_contacts_phone_idx" ON "public"."user_contacts" USING "btree" ("phone");



CREATE INDEX "user_contacts_user_id_idx" ON "public"."user_contacts" USING "btree" ("user_id");



CREATE INDEX "users_embedding_idx" ON "public"."users" USING "ivfflat" ("embedding" "public"."vector_cosine_ops") WITH ("lists"='100');



ALTER TABLE ONLY "public"."activity"
    ADD CONSTRAINT "activity_related_user_id_fkey" FOREIGN KEY ("related_user_id") REFERENCES "public"."users"("id");



ALTER TABLE ONLY "public"."activity"
    ADD CONSTRAINT "activity_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."box_items"
    ADD CONSTRAINT "box_items_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."card_views"
    ADD CONSTRAINT "card_views_viewed_id_fkey" FOREIGN KEY ("viewed_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."card_views"
    ADD CONSTRAINT "card_views_viewer_id_fkey" FOREIGN KEY ("viewer_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."connections"
    ADD CONSTRAINT "connections_initiated_by_fkey" FOREIGN KEY ("initiated_by") REFERENCES "public"."users"("id");



ALTER TABLE ONLY "public"."connections"
    ADD CONSTRAINT "connections_user_a_fkey" FOREIGN KEY ("user_a") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."connections"
    ADD CONSTRAINT "connections_user_b_fkey" FOREIGN KEY ("user_b") REFERENCES "public"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."notifications"
    ADD CONSTRAINT "notifications_related_user_fkey" FOREIGN KEY ("related_user") REFERENCES "public"."users"("id");
...
ALTER TABLE ONLY "public"."proximity_events"
    ADD CONSTRAINT "proximity_events_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."taste_items"
    ADD CONSTRAINT "taste_items_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;
...
CREATE POLICY "activity_read_own" ON "public"."activity" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "blocks_manage" ON "public"."blocks" USING (("auth"."uid"() = "blocker_id"));
...
CREATE POLICY "box_delete" ON "public"."box_items" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "box_insert" ON "public"."box_items" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "box_select" ON "public"."box_items" FOR SELECT USING (("auth"."role"() = 'authenticated'::"text"));



CREATE POLICY "box_update" ON "public"."box_items" FOR UPDATE USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "card_views_insert" ON "public"."card_views" FOR INSERT WITH CHECK (("auth"."uid"() = "viewer_id"));



CREATE POLICY "card_views_read_viewed" ON "public"."card_views" FOR SELECT USING (("auth"."uid"() = "viewed_id"));
...
CREATE POLICY "connection_requests_read" ON "public"."connection_requests" FOR SELECT USING ((("auth"."uid"() = "sender_id") OR ("auth"."uid"() = "receiver_id")));



CREATE POLICY "connection_requests_respond" ON "public"."connection_requests" FOR UPDATE USING (("auth"."uid"() = "receiver_id"));



CREATE POLICY "connection_requests_send" ON "public"."connection_requests" FOR INSERT WITH CHECK (("auth"."uid"() = "sender_id"));
...
CREATE POLICY "connections_insert" ON "public"."connections" FOR INSERT WITH CHECK (("auth"."uid"() = "initiated_by"));



CREATE POLICY "connections_insert_own" ON "public"."connections" FOR INSERT WITH CHECK ((("auth"."uid"() = "user_a") OR ("auth"."uid"() = "user_b")));



CREATE POLICY "connections_read_own" ON "public"."connections" FOR SELECT USING ((("auth"."uid"() = "user_a") OR ("auth"."uid"() = "user_b")));
...
CREATE POLICY "cr_update" ON "public"."connection_requests" FOR UPDATE USING (("auth"."uid"() = "receiver_id"));



CREATE POLICY "cv_insert" ON "public"."card_views" FOR INSERT WITH CHECK (("auth"."uid"() = "viewer_id"));



CREATE POLICY "cv_select" ON "public"."card_views" FOR SELECT USING ((("auth"."uid"() = "viewed_id") OR ("auth"."uid"() = "viewer_id")));
...
CREATE POLICY "device_tokens_own" ON "public"."device_tokens" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "dt_manage" ON "public"."device_tokens" USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "pe_insert" ON "public"."proximity_events" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "presence_manage_own" ON "public"."presence" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "presence_read_open" ON "public"."presence" FOR SELECT USING (("is_open" = true));



CREATE POLICY "presence_select" ON "public"."presence" FOR SELECT USING (("auth"."role"() = 'authenticated'::"text"));
...
CREATE POLICY "proximity_events_own" ON "public"."proximity_events" USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "taste_delete" ON "public"."taste_items" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "taste_insert" ON "public"."taste_items" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "taste_select" ON "public"."taste_items" FOR SELECT USING (("auth"."role"() = 'authenticated'::"text"));



CREATE POLICY "taste_update" ON "public"."taste_items" FOR UPDATE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "uc_delete" ON "public"."user_contacts" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "uc_insert" ON "public"."user_contacts" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "uc_select" ON "public"."user_contacts" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "users_read_connections" ON "public"."users" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."connections"
  WHERE ((("connections"."user_a" = "auth"."uid"()) AND ("connections"."user_b" = "connections"."id")) OR (("connections"."user_b" = "auth"."uid"()) AND ("connections"."user_a" = "connections"."id"))))));
...
GRANT ALL ON FUNCTION "public"."archive_thought"("p_thought_id" "uuid", "p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."archive_thought"("p_thought_id" "uuid", "p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."archive_thought"("p_thought_id" "uuid", "p_user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."award_points_busyness_report"() TO "anon";
GRANT ALL ON FUNCTION "public"."award_points_busyness_report"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."award_points_busyness_report"() TO "service_role";



GRANT ALL ON FUNCTION "public"."award_points_collectible_collected"() TO "anon";
GRANT ALL ON FUNCTION "public"."award_points_collectible_collected"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."award_points_collectible_collected"() TO "service_role";



GRANT ALL ON FUNCTION "public"."award_points_comment_created"() TO "anon";
GRANT ALL ON FUNCTION "public"."award_points_comment_created"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."award_points_comment_created"() TO "service_role";



GRANT ALL ON FUNCTION "public"."award_points_friendship_accepted"() TO "anon";
GRANT ALL ON FUNCTION "public"."award_points_friendship_accepted"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."award_points_friendship_accepted"() TO "service_role";



GRANT ALL ON FUNCTION "public"."award_points_moment_created"() TO "anon";
GRANT ALL ON FUNCTION "public"."award_points_moment_created"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."award_points_moment_created"() TO "service_role";



GRANT ALL ON FUNCTION "public"."award_points_moment_liked"() TO "anon";
GRANT ALL ON FUNCTION "public"."award_points_moment_liked"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."award_points_moment_liked"() TO "service_role";



GRANT ALL ON FUNCTION "public"."award_points_moment_viewed"() TO "anon";
GRANT ALL ON FUNCTION "public"."award_points_moment_viewed"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."award_points_moment_viewed"() TO "service_role";



GRANT ALL ON FUNCTION "public"."award_points_spot_tip"() TO "anon";
GRANT ALL ON FUNCTION "public"."award_points_spot_tip"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."award_points_spot_tip"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."can_message"("p_from_user" "uuid", "p_to_user" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."can_message"("p_from_user" "uuid", "p_to_user" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."can_message"("p_from_user" "uuid", "p_to_user" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."check_in_to_spot"("p_spot_id" "uuid", "p_user_id" "uuid", "p_busyness_level" integer, "p_note" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."check_in_to_spot"("p_spot_id" "uuid", "p_user_id" "uuid", "p_busyness_level" integer, "p_note" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_in_to_spot"("p_spot_id" "uuid", "p_user_id" "uuid", "p_busyness_level" integer, "p_note" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."compute_user_similarity"() TO "anon";
GRANT ALL ON FUNCTION "public"."compute_user_similarity"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."compute_user_similarity"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."create_default_archive_collection"() TO "anon";
GRANT ALL ON FUNCTION "public"."create_default_archive_collection"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_default_archive_collection"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."expire_sotds"() TO "anon";
GRANT ALL ON FUNCTION "public"."expire_sotds"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."expire_sotds"() TO "service_role";



GRANT ALL ON FUNCTION "public"."find_mutual_contacts"("current_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."find_mutual_contacts"("current_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."find_mutual_contacts"("current_user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_all_collections"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_all_collections"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_all_collections"() TO "service_role";



GRANT ALL ON FUNCTION "public"."get_campus_spots_near"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_accessible_only" boolean) TO "anon";
GRANT ALL ON FUNCTION "public"."get_campus_spots_near"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_accessible_only" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_campus_spots_near"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_accessible_only" boolean) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_challenge_completions"("p_challenge_id" "uuid", "p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_challenge_completions"("p_challenge_id" "uuid", "p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_challenge_completions"("p_challenge_id" "uuid", "p_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_challenges_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_challenges_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_challenges_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_collectibles_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_collectibles_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_collectibles_near_location"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school_id" "text", "p_user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_friend_moments"("requesting_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_friend_moments"("requesting_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_friend_moments"("requesting_user_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_friend_of_friend_candidates"("p_user_id" "uuid", "p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_friend_of_friend_candidates"("p_user_id" "uuid", "p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_friend_of_friend_candidates"("p_user_id" "uuid", "p_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_friendship_statuses_bulk"("p_user_id" "uuid", "p_target_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."get_friendship_statuses_bulk"("p_user_id" "uuid", "p_target_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_friendship_statuses_bulk"("p_user_id" "uuid", "p_target_ids" "uuid"[]) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_oracle_recommendations"("p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_oracle_recommendations"("p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_oracle_recommendations"("p_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_popular_spots_nearby"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_popular_spots_nearby"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_popular_spots_nearby"("p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_profile_compliments"("target_profile_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_profile_compliments"("target_profile_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_profile_compliments"("target_profile_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_social_graph"("p_user_id" "uuid", "p_depth" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_social_graph"("p_user_id" "uuid", "p_depth" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_social_graph"("p_user_id" "uuid", "p_depth" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_spot_tips"("p_spot_id" "uuid", "p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_spot_tips"("p_spot_id" "uuid", "p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_spot_tips"("p_spot_id" "uuid", "p_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_suggested_profiles"("p_user_id" "uuid", "p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_suggested_profiles"("p_user_id" "uuid", "p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_suggested_profiles"("p_user_id" "uuid", "p_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_top_rooms"("limit_count" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_top_rooms"("limit_count" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_top_rooms"("limit_count" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_user_collectibles"("p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_user_collectibles"("p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_user_collectibles"("p_user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."increment_invites"("uid" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."increment_invites"("uid" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."increment_invites"("uid" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."increment_moment_like"("p_moment_id" "uuid", "p_user_id" "uuid", "p_amount" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."increment_moment_like"("p_moment_id" "uuid", "p_user_id" "uuid", "p_amount" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."increment_moment_like"("p_moment_id" "uuid", "p_user_id" "uuid", "p_amount" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."increment_moment_views"() TO "anon";
GRANT ALL ON FUNCTION "public"."increment_moment_views"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."increment_moment_views"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."log_interaction"("p_moment_id" "uuid", "p_action" "text", "p_duration_ms" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."log_interaction"("p_moment_id" "uuid", "p_action" "text", "p_duration_ms" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."log_interaction"("p_moment_id" "uuid", "p_action" "text", "p_duration_ms" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."record_thought_view"("p_thought_id" "uuid", "p_viewer_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."record_thought_view"("p_thought_id" "uuid", "p_viewer_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."record_thought_view"("p_thought_id" "uuid", "p_viewer_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."search_connections"("query_embedding" "public"."vector", "current_user_id" "uuid", "match_count" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."search_connections"("query_embedding" "public"."vector", "current_user_id" "uuid", "match_count" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_connections"("query_embedding" "public"."vector", "current_user_id" "uuid", "match_count" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."search_moments_v2"("p_query_embedding" "public"."vector", "p_user_id" "uuid", "p_match_threshold" double precision, "p_match_count" integer, "p_date_from" timestamp with time zone, "p_date_to" timestamp with time zone, "p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."search_moments_v2"("p_query_embedding" "public"."vector", "p_user_id" "uuid", "p_match_threshold" double precision, "p_match_count" integer, "p_date_from" timestamp with time zone, "p_date_to" timestamp with time zone, "p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_moments_v2"("p_query_embedding" "public"."vector", "p_user_id" "uuid", "p_match_threshold" double precision, "p_match_count" integer, "p_date_from" timestamp with time zone, "p_date_to" timestamp with time zone, "p_lat" double precision, "p_lng" double precision, "p_radius_miles" double precision, "p_school" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."search_oracle_profiles"("p_tags" "text"[], "p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."search_oracle_profiles"("p_tags" "text"[], "p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_oracle_profiles"("p_tags" "text"[], "p_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."search_people_semantic"("p_requesting_user_id" "uuid", "p_query" "text", "p_user_lat" double precision, "p_user_lon" double precision, "p_radius_miles" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."search_people_semantic"("p_requesting_user_id" "uuid", "p_query" "text", "p_user_lat" double precision, "p_user_lon" double precision, "p_radius_miles" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_people_semantic"("p_requesting_user_id" "uuid", "p_query" "text", "p_user_lat" double precision, "p_user_lon" double precision, "p_radius_miles" double precision) TO "service_role";



GRANT ALL ON FUNCTION "public"."search_profiles_v2"("search_query" "text", "match_count" integer, "exclude_user_id" "uuid", "filter_gender" "text", "filter_school" "text", "filter_ethnicity" "text"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."search_profiles_v2"("search_query" "text", "match_count" integer, "exclude_user_id" "uuid", "filter_gender" "text", "filter_school" "text", "filter_ethnicity" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_profiles_v2"("search_query" "text", "match_count" integer, "exclude_user_id" "uuid", "filter_gender" "text", "filter_school" "text", "filter_ethnicity" "text"[]) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_moment_comment_count"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_moment_comment_count"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_moment_comment_count"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."upsert_chunk_visit"("p_user_id" "uuid", "p_chunk_id" "text", "p_source" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."upsert_chunk_visit"("p_user_id" "uuid", "p_chunk_id" "text", "p_source" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."upsert_chunk_visit"("p_user_id" "uuid", "p_chunk_id" "text", "p_source" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."upsert_user_location"("p_user_id" "uuid", "p_latitude" double precision, "p_longitude" double precision, "p_accuracy" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."upsert_user_location"("p_user_id" "uuid", "p_latitude" double precision, "p_longitude" double precision, "p_accuracy" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."upsert_user_location"("p_user_id" "uuid", "p_latitude" double precision, "p_longitude" double precision, "p_accuracy" double precision) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."verify_admin_password"("pw" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."verify_admin_password"("pw" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."verify_admin_password"("pw" "text") TO "service_role";
...
GRANT ALL ON TABLE "public"."box_items" TO "anon";
GRANT ALL ON TABLE "public"."box_items" TO "authenticated";
GRANT ALL ON TABLE "public"."box_items" TO "service_role";



GRANT ALL ON TABLE "public"."card_views" TO "anon";
GRANT ALL ON TABLE "public"."card_views" TO "authenticated";
GRANT ALL ON TABLE "public"."card_views" TO "service_role";
...
GRANT ALL ON TABLE "public"."proximity_events" TO "anon";
GRANT ALL ON TABLE "public"."proximity_events" TO "authenticated";
GRANT ALL ON TABLE "public"."proximity_events" TO "service_role";
...
GRANT ALL ON TABLE "public"."taste_items" TO "anon";
GRANT ALL ON TABLE "public"."taste_items" TO "authenticated";
GRANT ALL ON TABLE "public"."taste_items" TO "service_role";
...
using ((bucket_id = 'card-images'::text));
  ...
  create policy "Chat media are publicly readable"
  on "storage"."objects"
  as permissive
  for select
  to public
...
using (((bucket_id = 'posts'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
  ...
  for update
  to authenticated
using (((bucket_id = 'card-images'::text) AND ((storage.foldername(name))[1] = 'cards'::text) AND ((storage.foldername(name))[2] = (auth.uid())::text)));
...
using (((bucket_id = 'posts'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
  ...
  for insert
  to public
with check (((bucket_id = 'chat-media'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
  ...
  for insert
  to authenticated
with check (((bucket_id = 'card-images'::text) AND ((storage.foldername(name))[1] = 'cards'::text) AND ((storage.foldername(name))[2] = (auth.uid())::text)));
  ...
  for insert
  to public
with check (((bucket_id = 'ids'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
...
with check (((bucket_id = 'posts'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
  ...
  for select
  to public
using (((bucket_id = 'ids'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
